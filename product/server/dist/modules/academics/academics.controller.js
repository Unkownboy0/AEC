...
class AcademicsController {
    service = new academics_service_1.AcademicsService();
    // ==========================================
    // DASHBOARD STATS
    // ==========================================
    ...
    createDept = async (req, res, next) => {
        try {
            ...
            await (0, security_1.auditRequest)(req, 'CREATE', 'DEPARTMENTS', `Created Department: ${dept.name} (${dept.code})`, dept.id, 'Department');
    ...
    updateDept = async (req, res, next) => {
        try {
            ...
            await (0, security_1.auditRequest)(req, 'UPDATE', 'DEPARTMENTS', `Updated Department details for: ${dept.code}`, dept.id, 'Department');
            res.status(200).json({ status: 'success', data: dept });
        }
        catch (error) {
            next(error);
    ...
    // SEMESTERS
    // ==========================================
    listSemesters = async (req, res, next) => {
        try {
            const user = req.user;
    ...
    bulkAction = async (req, res, next) => {
        try {
            ...
            if (!moduleKey || !action || !ids || !Array.isArray(ids)) {
                throw new exceptions_1.BadRequestException('moduleKey, action, and ids array are required');
            }
            const user = req.user;
            if (action === 'delete') {
                ...
                throw new exceptions_1.BadRequestException('Invalid bulk action');
            }
    ...
    importPreview = async (req, res, next) => {
        try {
    ...
    importCommit = async (req, res, next) => {
        try {
            const { moduleKey, rows } = req.body;
            const user = req.user;
    ...
    // ==========================================
    listSubjectAssignments = async (req, res, next) => {
        try {
            const user = req.user;
            ...
            if (departmentId) {
                filters.subject = { ...(filters.subject || {}), departmentId: departmentId };
            }
            ...
            const items = await prisma_1.prisma.subjectAssignment.findMany({
                where: filters,
                include: {
    ...
    createSubjectAssignment = async (req, res, next) => {
        try {
            const user = req.user;
            ...
            const existing = await prisma_1.prisma.subjectAssignment.findFirst({
            ...
            const item = await prisma_1.prisma.subjectAssignment.create({
                data: {
                    facultyId,
                    subjectId,
                    sectionId,
    ...
    deleteSubjectAssignment = async (req, res, next) => {
        try {
            const { id } = req.params;
            await prisma_1.prisma.subjectAssignment.delete({ where: { id } });
    ...
    facultyCreateSubjectAssignment = async (req, res, next) => {
        try {
            const user = req.user;
            const { departmentId, programId, academicYearId, semesterId, sectionId, subjectCode, subjectName, subjectType, credits, teachingHours } = req.body;
            ...
            let subject = await prisma_1.prisma.subject.findFirst({
            ...
            const assignment = await prisma_1.prisma.subjectAssignment.create({
                data: {
                    facultyId: faculty.id,
                    subjectId: subject.id,
                    sectionId,
    ...
    facultyUpdateSubjectAssignment = async (req, res, next) => {
        try {
            const user = req.user;
            const { id } = req.params;
            ...
            const assignment = await prisma_1.prisma.subjectAssignment.findUnique({
                where: { id },
            ...
            const updatedAssignment = await prisma_1.prisma.subjectAssignment.update({
                where: { id },
                data: updateData,
            ...
            if (assignment.subjectId) {
                const isLab = subjectType === 'Lab';
                const isElective = subjectType === 'Elective';
                await prisma_1.prisma.subject.update({
    ...
    facultyDeleteSubjectAssignment = async (req, res, next) => {
        try {
            const user = req.user;
            const { id } = req.params;
            const faculty = await prisma_1.prisma.faculty.findFirst({ where: { userId: user.id } });
