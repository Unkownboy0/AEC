...
export async function generateAttendanceReportPdf(data: AttendanceReportData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    const subjectsMap: Record<string, { id: string, code: string, name: string, faculty: string, total: number, present: number, absent: number }> = {};
    ...
    const monthsMap: Record<string, { name: string, total: number, present: number, absent: number }> = {};
    attendanceRecords.forEach((r) => {
    ...
    doc.fillColor(overallPercent >= 75 ? '#065f46' : '#9f1239')
       ...
       .text(overallPercent >= 75
         ...
         : `ALERT INELIGIBLE - Low attendance alert! Overall attendance is ${overallPercent}%, which falls below the mandatory 75% examination threshold. Please consult your Class Advisor Mrs. Ada Lovelace immediately.`,
         50, currentY + 22);
    doc.restore();
