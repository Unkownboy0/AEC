 ...
 */
function drawPrincipalSignature(doc: PDFKit.PDFDocument, x: number, y: number) {
  doc.save();
 ...
 */
function drawBarcode(doc: PDFKit.PDFDocument, x: number, y: number, height: number) {
  doc.save();
 ...
 */
export async function buildStudentIDCardPDF(data: IDCardData): Promise<PDFKit.PDFDocument> {
  ...
  try {
    ...
    throw new BadRequestException('Dynamic QR Code generation failed.');
  }
  ...
  if (settings.PRINCIPAL_SIGNATURE) {
    try {
      ...
      if (rawPath.startsWith('data:image')) {
        ...
        const localPath = path.join(process.cwd(), cleanPath);
        if (fs.existsSync(localPath)) {
          signatureBuffer = fs.readFileSync(localPath);
        }
      }
    } catch (err) {
