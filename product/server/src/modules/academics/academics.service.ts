...
export class AcademicsService {
  ...
  // ==========================================
  async getFacultyDeptId(userId: string): Promise<string | null> {
    const f = await prisma.faculty.findFirst({ where: { userId } });
    if (f?.departmentId) return f.departmentId;

    const mem = await prisma.departmentMembership.findFirst({ where: { userId } });
    if (mem?.departmentId) return mem.departmentId;

    const usr = await prisma.user.findUnique({ where: { id: userId } });
    return usr?.departmentId || null;
  }

  async listDepartments(params: any, user?: UserPayload) {
  ...
  async createDepartment(input: any, userId: string, ip?: string, ua?: string) {
    ...
    // Check uniqueness
    const existingCode = await prisma.department.findFirst({ where: { code, deleted: false } });
    if (existingCode) throw new BadRequestException('Department Code already exists');
  ...
  async updateDepartment(id: string, input: any, userId: string, ip?: string, ua?: string) {
    ...
    if (officeLocation !== undefined) data.officeLocation = officeLocation;
    if (establishedYear !== undefined) data.establishedYear = establishedYear ? parseInt(establishedYear) : null;
    if (hodName !== undefined) data.hodName = hodName;
  ...
  // 2. PROGRAMS SERVICE
  // ==========================================
  async listPrograms(params: any, user?: UserPayload) {
    const page = Math.max(1, parseInt(params.page) || 1);
  ...
  async createProgram(input: any, userId: string, ip?: string, ua?: string) {
    const { code, name, duration, level, credits, departmentId, coordinator, status } = input;
    if (!code || !name || !departmentId) throw new BadRequestException('Code, Name and Department are required');
  ...
  async updateProgram(id: string, input: any, userId: string, ip?: string, ua?: string) {
  ...
  // 3. COURSES SERVICE
  // ==========================================
  async listCourses(params: any, user?: UserPayload) {
    const page = Math.max(1, parseInt(params.page) || 1);
    const pageSize = Math.max(1, parseInt(params.pageSize) || 10);
  ...
  async createCourse(input: any, userId: string, ip?: string, ua?: string) {
    ...
    if (!code || !name || !programId || !departmentId) {
      throw new BadRequestException('Code, Name, Program, and Department are required');
    }
  ...
  async updateCourse(id: string, input: any, userId: string, ip?: string, ua?: string) {
    ...
    if (duration !== undefined) data.duration = parseInt(duration);
    if (credits !== undefined) data.credits = parseInt(credits);
    if (regulation) data.regulation = regulation;
    if (coordinator !== undefined) data.coordinator = coordinator;
    if (description !== undefined) data.description = description;
    if (status) data.status = status;
  ...
  async createYear(input: any, userId: string, ip?: string, ua?: string) {
  ...
  async updateYear(id: string, input: any, userId: string, ip?: string, ua?: string) {
  ...
  async cloneAcademicYear(id: string, targetName: string, userId: string, ip?: string, ua?: string) {
    const sourceYear = await this.getYear(id);
    
    // Check if target year name already registered
    const existing = await prisma.academicYear.findUnique({ where: { name: targetName } });
    if (existing) throw new BadRequestException(`Academic Year '${targetName}' already exists.`);
  ...
  // 5. SEMESTERS SERVICE
  // ==========================================
  async listSemesters(params: any, user?: UserPayload) {
    const page = Math.max(1, parseInt(params.page) || 1);
    const pageSize = Math.max(1, parseInt(params.pageSize) || 10);
  ...
  async createSemester(input: any, userId: string, ip?: string, ua?: string) {
    ...
    if (!number || !courseId || !programId || !academicYearId) {
      throw new BadRequestException('Number, Course, Program, and Academic Year are required');
    }
  ...
  async updateSemester(id: string, input: any, userId: string, ip?: string, ua?: string) {
  ...
  // 6. SECTIONS SERVICE
  // ==========================================
  async listSections(params: any, user?: UserPayload) {
    const page = Math.max(1, parseInt(params.page) || 1);
    const pageSize = Math.max(1, parseInt(params.pageSize) || 10);
  ...
  async createSection(input: any, userId: string, ip?: string, ua?: string) {
    ...
    if (!name || !semesterId || !programId || !departmentId) {
      throw new BadRequestException('Name, Semester, Program, and Department are required');
    }
  ...
  async updateSection(id: string, input: any, userId: string, ip?: string, ua?: string) {
    ...
    if (classAdvisor !== undefined) data.classAdvisor = classAdvisor;
  ...
  // 7. SUBJECTS SERVICE
  // ==========================================
  async listSubjects(params: any, user?: UserPayload) {
    const page = Math.max(1, parseInt(params.page) || 1);
    const pageSize = Math.max(1, parseInt(params.pageSize) || 10);
  ...
  async createSubject(input: any, userId: string, ip?: string, ua?: string) {
    const {
      name, code, credits, theoryHours, practicalHours, tutorialHours,
      internalMarks, externalMarks, passingMarks, isElective, isLab, isCore,
      isSkillBased, isOpenElective, isProfessionalElective, subjectCoordinator,
      description, status, semesterId, departmentId, programId, sectionId
    } = input;

    if (!name || !code || !semesterId || !departmentId || !programId) {
      throw new BadRequestException('Name, Code, Semester, Department, and Program are required');
    }

    const existing = await prisma.subject.findFirst({ where: { code, deleted: false } });
    if (existing) throw new BadRequestException(`Subject Code '${code}' already registered.`);
  ...
  async updateSubject(id: string, input: any, userId: string, ip?: string, ua?: string) {
    ...
    if (code) data.code = code;
    if (credits !== undefined) data.credits = parseInt(credits);
    if (theoryHours !== undefined) data.theoryHours = parseInt(theoryHours);
    if (practicalHours !== undefined) data.practicalHours = parseInt(practicalHours);
    if (tutorialHours !== undefined) data.tutorialHours = parseInt(tutorialHours);
    if (internalMarks !== undefined) data.internalMarks = parseInt(internalMarks);
    if (externalMarks !== undefined) data.externalMarks = parseInt(externalMarks);
    if (passingMarks !== undefined) data.passingMarks = parseInt(passingMarks);
    if (isElective !== undefined) data.isElective = !!isElective;
    if (isLab !== undefined) data.isLab = !!isLab;
    if (isCore !== undefined) data.isCore = !!isCore;
    if (isSkillBased !== undefined) data.isSkillBased = !!isSkillBased;
    if (isOpenElective !== undefined) data.isOpenElective = !!isOpenElective;
    if (isProfessionalElective !== undefined) data.isProfessionalElective = !!isProfessionalElective;
    if (subjectCoordinator !== undefined) data.subjectCoordinator = subjectCoordinator;
    if (description !== undefined) data.description = description;
    if (status) data.status = status;
    if (semesterId) data.semesterId = semesterId;
    if (departmentId) data.departmentId = departmentId;
  ...
  async getDashboardStats() {
    const [depts, progs, courses, years, sems, secs, subs] = await Promise.all([
      prisma.department.count({ where: { deleted: false, archived: false } }),
      prisma.program.count({ where: { deleted: false, archived: false } }),
      prisma.course.count({ where: { deleted: false, archived: false } }),
      prisma.academicYear.count({ where: { deleted: false, archived: false } }),
      prisma.semester.count({ where: { deleted: false, archived: false } }),
      prisma.section.count({ where: { deleted: false, archived: false } }),
      prisma.subject.count({ where: { deleted: false, archived: false } }),
    ]);

    // Live real-time database counts
    const pendingApprovalsCount = await (prisma as any).approvalAssignment.count({
      where: { status: 'PENDING' },
    });

    let totalStudentsCount = 0;
    try {
      totalStudentsCount = await (prisma as any).student.count({
    ...
    try {
      totalFacultyCount = await (prisma as any).faculty.count({
        where: { status: 'ACTIVE' },
      });
    } catch (err) {}

    let deptsWithCounts: any[] = [];
    try {
      deptsWithCounts = await (prisma as any).department.findMany({
    ...
    const deptsDistribution = await prisma.department.groupBy({
      by: ['type'],
      _count: { id: true },
      where: { deleted: false, archived: false },
    });

    const levelDistribution = await prisma.program.groupBy({
      by: ['level'],
      _count: { id: true },
      where: { deleted: false, archived: false },
    });
    ...
    return {
      ...
      departments: deptsWithCounts.map((d: any) => ({
        id: d.id,
        name: d.name,
        code: d.code,
  ...
  // ==========================================
  async previewImport(moduleKey: string, rows: any[]) {
    const report: { valid: any[]; duplicates: any[]; errors: any[] } = {
      valid: [],
      duplicates: [],
      errors: [],
    };
    ...
    for (let idx = 0; idx < rows.length; idx++) {
      ...
      try {
        if (moduleKey === 'departments') {
          ...
          if (!code || !name) throw new Error('Code and Name are mandatory');
          
          const duplicate = await prisma.department.findFirst({ where: { code, deleted: false } });
          if (duplicate) {
          ...
          const duplicate = await prisma.subject.findFirst({ where: { code, deleted: false } });
          if (duplicate) {
          ...
          if (!row.code && !row.name) throw new Error('Both Code and Name must be defined');
          report.valid.push(row);
        }
      } catch (err: any) {
  ...
  async executeImport(moduleKey: string, rows: any[], userId: string, ip?: string, ua?: string) {
    let count = 0;
    ...
    if (moduleKey === 'departments') {
      ...
      if (!defaultDept) throw new BadRequestException('Create at least one Department before importing programs');
      for (const row of rows) {
      ...
      const defaultSem = await prisma.semester.findFirst({ where: { deleted: false } });
      const defaultDept = await prisma.department.findFirst({ where: { deleted: false } });
      const defaultProg = await prisma.program.findFirst({ where: { deleted: false } });
      
      if (!defaultSem || !defaultDept || !defaultProg) {
        throw new BadRequestException('Academics tables (Semesters, Depts, Programs) must be seeded first');
      }
      ...
      throw new BadRequestException(`Import not implemented for module: ${moduleKey}`);
    }
