...
export class CertificateController {
  ...
  download = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const requestorStudent = await prisma.student.findFirst({ where: { userId: requestor?.id, deleted: false }, select: { id: true } });
      const staffRoles = ['Super Admin', 'College Admin', 'Principal', 'Examination Cell', 'COE'];
      if (requestorStudent?.id !== certificate.studentId && !staffRoles.includes(String(requestor?.role || ''))) {
        throw new ForbiddenException('You are not authorized to download this certificate');
      }
      ...
      const buffer = await new Promise<Buffer>((resolve, reject) => {
        doc.on('end', () => {
          ...
          if (!result || result.length === 0) return reject(new BadRequestException('Generated certificate PDF is empty.'));
          if (result.slice(0, 5).toString() !== '%PDF-') return reject(new BadRequestException('Invalid certificate PDF signature.'));
          resolve(result);
        });
        doc.on('error', reject);
        doc.end();
