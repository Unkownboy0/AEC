...
const PROJECTS: Project[] = [
  {
    id: "1",
    title: "GEETORUS CampusOS – Enterprise ERP",
    description:
      "A full-stack enterprise-grade campus management system handling student lifecycle, faculty operations, admissions, placements, and institutional analytics. Built with React, Node.js, Prisma, and PostgreSQL.",
    tech: ["React", "TypeScript", "Node.js", "Prisma", "PostgreSQL", "JWT"],
    status: "COMPLETED",
    github: "https://github.com",
    demo: "https://campusos.geetorus.edu.in",
    guide: "Dr. Sarah Chen",
    type: "Major",
    year: "2026",
    stars: 48,
  },
  {
    ...
    description:
      "A browser-based IDE with live multi-user collaboration powered by WebSockets and CRDT. Supports 12+ languages with syntax highlighting and auto-completion using Monaco Editor.",
    tech: ["React", "WebSocket", "Node.js", "Monaco", "Redis"],
    status: "COMPLETED",
    github: "https://github.com",
    guide: "Prof. Mark Davis",
    type: "Minor",
    year: "2025",
    stars: 31,
  },
  {
    id: "3",
    title: "AI-Powered Resume Scoring Engine",
    description:
      "Trained a BERT-based NLP model to evaluate resume ATS scores, keyword relevance, grammar quality, and role-fit probability. Deployed as a REST API with FastAPI.",
    tech: ["Python", "PyTorch", "BERT", "FastAPI", "Docker"],
    status: "IN_PROGRESS",
    github: "https://github.com",
    type: "Personal",
    year: "2026",
  ...
  {
    id: "4",
    title: "Distributed Cache Invalidation System",
    description:
      "Research prototype exploring cache coherence protocols in distributed key-value stores. Implemented three strategies: TTL, LRU, and event-sourced invalidation patterns.",
    tech: ["Go", "Redis", "Kafka", "Docker", "Kubernetes"],
    status: "IN_PROGRESS",
    type: "Research",
    year: "2026",
  },
  ...
  return (
    <div className="space-y-6 text-left pb-12 animate-in fade-in duration-200">
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        <div>
          <h1 className="text-xl font-black text-slate-800 dark:text-white flex items-center gap-2">
            <FolderGit className="h-5 w-5 text-indigo-600" /> Engineering
            Portfolio
          </h1>
          <p className="text-xs text-muted-foreground mt-0.5">
            Showcase your projects, research, and open-source contributions
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            value: projects.reduce((a, p) => a + (p.stars || 0), 0),
          },
        ].map((s, idx) => (
          <div
            key={idx}
            className="border bg-card p-4 rounded-2xl shadow-sm text-center"
          >
          ...
          >
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                className="w-full px-3 py-2 border rounded-xl bg-background outline-none resize-none"
              />
            </div>
            <div>
              <label className="block text-[8px] uppercase font-black text-slate-500 mb-1">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewProject((p) => ({ ...p, github: e.target.value }))
                }
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 border rounded-xl bg-background outline-none"
              >
                {["Major", "Minor", "Personal", "Research"].map((t) => (
            ...
            <div className="md:col-span-2 flex gap-2">
              <button
                type="submit"
                className="px-4 py-2 bg-indigo-600 text-white font-black rounded-xl text-xs"
              >
                Add Project
              </button>
              <button
                ...
                className="px-4 py-2 border font-black rounded-xl text-xs hover:bg-muted"
              >
                Cancel
              </button>
            </div>
      ...
      <div className="flex gap-2 flex-wrap text-[10px] font-black">
        {(["ALL", "Major", "Minor", "Personal", "Research"] as const).map(
          (f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {filtered.map((p) => (
          <div
            key={p.id}
            className="border bg-card rounded-2xl shadow-sm p-5 space-y-3 hover:shadow-md transition-all group"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="flex gap-2 items-center flex-wrap">
                <span
                  className={`text-[8.5px] font-black uppercase px-2 py-0.5 rounded ${TYPE_COLORS[p.type]}`}
                >
                  {p.type}
                </span>
                <span
                  className={`text-[8.5px] font-black uppercase px-2 py-0.5 rounded border ${STATUS_COLORS[p.status]}`}
                >
                  {p.status.replace("_", " ")}
                </span>
              </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {p.tech.map((t) => (
                <span
                  key={t}
                  className="bg-muted text-slate-600 dark:text-slate-300 text-[9px] font-bold px-2 py-0.5 rounded-full font-mono"
                >
                  {t}
                </span>
              ))}
