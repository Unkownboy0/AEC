...
export const StudentCareerDashboard: React.FC = () => {
  ...
  const interviewQuestions = [
    ...
    {
      q: "Explain the difference between optimistic and pessimistic locking in database transaction isolation.",
      ans: "Optimistic locking checks for conflict before commit, pessimistic locks records immediately.",
    },
    {
      q: "Why is Git rebasing preferred over merging in large collaborate workflows?",
      ans: "It maintains a clean linear commit history.",
    },
  ];
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] uppercase font-black tracking-wider text-cyan-400 bg-cyan-950/40 border border-cyan-800/40 px-2 py-0.5 rounded flex items-center gap-1">
              <Sparkles className="h-3 w-3 animate-pulse" /> Employability
              Analytics
            </span>
          </div>
          <h1 className="text-2xl font-black text-white mt-1.5 flex items-center gap-2">
            <TrendingUp className="h-6 w-6 text-indigo-500" /> Career &
            Readiness Center
          </h1>
          <p className="text-xs text-slate-400">
            Track coding metrics, mock interview history, alumni network, and
            job offers
          </p>
        </div>
      </div>
        ...
        {[
          {
            ...
            value: "94 / 100",
            sub: "High Probability Class Tier",
            icon: <Sparkles className="h-4 w-4 text-cyan-400" />,
          },
          {
            label: "LeetCode Solved",
            value: `${codingStats.solved} Problems`,
            sub: `${codingStats.rank} global rank`,
            icon: <GitBranch className="h-4 w-4 text-emerald-400" />,
          },
          {
            label: "Mock Interview Grade",
            value: `${mockScore}%`,
          ...
          },
          {
            label: "Offers Mapped",
            value:
              offer.status === "ACCEPTED" ? "1 Accepted" : "1 Offer Received",
            sub: "CTC: 32.5 LPA",
          ...
          <div
            key={idx}
            className="border border-slate-800 bg-slate-900/50 p-4 rounded-xl shadow-md flex items-start justify-between"
          >
          ...
          <div className="border border-slate-800 bg-slate-900/40 p-5 rounded-xl space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-xs font-black uppercase text-slate-400 tracking-wider flex items-center gap-1.5">
                <Cpu className="h-4 w-4 text-indigo-400" /> AI Mock Interview
                Board
              </h3>
            ...
            {interviewStatus === "idle" && (
              <div className="text-left space-y-3">
                <p className="text-xs text-slate-400">
                  Simulate interactive technical/HR interview boards with
            ...
            {interviewStatus === "feedback" && (
              <div className="p-4 border border-indigo-950 bg-indigo-950/20 rounded-xl space-y-3 text-xs">
                <h4 className="font-extrabold text-white flex items-center gap-1">
                  <Sparkles className="h-4 w-4 text-cyan-400" /> AI Feedback
                  Summary
                </h4>
                <div className="space-y-2 leading-relaxed text-slate-300 font-semibold">
                  <p>
                    ✓ Excellent core data structure knowledge. Solved complexity
                    calculations correctly.
                  </p>
                  <p>
                    ✓ Speech rate: 135 wpm (Stable). Emotion index: Confidence
                    high (94%).
                  </p>
                  <p>
                    • Recommendation: Clarify transaction locks with specific
                    examples like database deadlocks.
                  </p>
                </div>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-4 border border-slate-800 bg-slate-950 rounded-xl space-y-3">
                ...
                <div className="h-10 flex gap-0.5 items-end justify-between border-b pb-1">
                  {[2, 5, 8, 4, 12, 18, 5, 14, 22, 10, 8, 16, 25, 4, 18].map(
                    (c, i) => (
                      <div
                        key={i}
                        className="flex-1 bg-emerald-600 rounded-xs hover:bg-emerald-500 cursor-pointer"
                ...
                <span className="text-[8px] text-slate-500 block text-right font-bold">
                  Latest 15 days contribution activity
                </span>
              </div>
            </div>
          </div>
              ...
              {offer.status === "PENDING" && (
                <div className="flex gap-2 pt-1 border-t border-slate-850">
                  <button
                    ...
                    className="flex-1 py-2 border border-slate-800 hover:bg-slate-900 rounded-lg text-slate-400 font-extrabold transition-colors"
                  >
                    Decline
                  </button>
                  <button
                    onClick={handleAcceptOffer}
                    className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-500 text-slate-950 rounded-lg font-black transition-colors"
                  >
                    Accept Offer
                  </button>
                </div>
          ...
          <div className="border border-slate-800 bg-slate-900/40 p-5 rounded-xl space-y-4 text-left">
            ...
            <div className="space-y-3">
              {ALUMNI_MENTORS.map((mentor, idx) => (
                <div
                  key={idx}
                  className="p-3 border border-slate-800 bg-slate-950 rounded-xl flex items-center justify-between text-xs"
                >
                  ...
                  <button
                    onClick={() =>
                      toast.success(
                        `Mentorship invitation sent to ${mentor.name}.`,
                      )
                    }
                    className="p-1.5 hover:bg-slate-900 border border-slate-800 rounded-lg text-slate-400 transition-colors"
                  >
