...
export const FacultyTransport: React.FC = () => {
  ...
  const fetchTransportData = useCallback(async (showIndicator = false) => {
    try {
      ...
      console.error('Failed to fetch faculty transport allocation:', err);
      setData({
        isEligible: false,
  ...
  const handleApplyBus = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Faculty College Bus pass application submitted to Transport Office!');
    } catch {
      toast.error('Failed to submit application.');
    }
  };

  if (isLoading) return <Loading text="Verifying Faculty Transport Allocation & GPS Status..." />;

  // ── 1. Non-College Bus View ─────────────────────────────────────────────
  if (data && !data.isEligible) {
    return (
      <div className="space-y-6 text-left pb-12 animate-in fade-in duration-200">
        <div>
          <h1 className="text-xl font-black text-slate-800 dark:text-white flex items-center gap-2">
            <Truck className="h-5 w-5 text-indigo-600" /> Faculty Campus Commute
          </h1>
          <p className="text-xs text-muted-foreground mt-0.5">
            Employee transport mode: <span className="font-bold text-slate-700 dark:text-slate-300">Self Commute / Unassigned</span>
          </p>
        </div>
        ...
        <div className="p-6 rounded-3xl border bg-card shadow-sm space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-extrabold text-slate-900 dark:text-white">
                Faculty College Bus Allocation
              </h2>
              <p className="text-xs text-muted-foreground">
                {data.message || 'You currently do not have an active College Bus pass allocated.'}
              </p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-300 space-y-2">
            <p>
              Faculty members and staff can avail dedicated air-conditioned and standard college bus services across all city routes. If you wish to avail or change your staff bus pass, submit a request below.
            </p>
          </div>
          ...
          {applicationSubmitted ? (
            <div className="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-xs font-semibold text-emerald-800 dark:text-emerald-300 flex items-center gap-2">
              ...
              Faculty Bus Pass application for {selectedRoute} ({preferredStop}) submitted to Transport Department.
            </div>
          ) : (
            <button
            ...
            >
              <Plus className="h-4 w-4" /> Request Staff Bus Pass
            </button>
          )}
        </div>
  ...
  // ── 2. Full Active Live Tracking View ──────────────────────────────────
  return (
    <div className="space-y-6 text-left pb-12 animate-in fade-in duration-200">
      {/* Header */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="p-4 rounded-2xl border bg-card shadow-xs space-y-1">
          <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
            <MapPin className="w-3.5 h-3.5 text-emerald-600" /> Faculty Boarding Stop
          </div>
        ...
        <div className="p-4 rounded-2xl border bg-card shadow-xs space-y-1">
          <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
            <Bus className="w-3.5 h-3.5 text-indigo-600" /> Assigned Bus
          </div>
          <div className="text-sm font-extrabold text-slate-900 dark:text-white">
            {data?.vehicle?.number || 'TN 38 BX 4421'}
          </div>
          <div className="text-xs text-muted-foreground pt-0.5">
            Type: {data?.vehicle?.type || 'College Bus'}
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl border bg-card shadow-xs space-y-1">
          <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
            <User className="w-3.5 h-3.5 text-blue-600" /> Driver Details
          </div>
          ...
          <div className="text-xs text-muted-foreground pt-0.5">
            Contact: {data?.driver?.phone || 'Available via Transport Desk'}
          </div>
        </div>
      </div>
